"""OpenAI native tools, end to end, streaming.

    uv run python specs/0009-provider-native-tools/examples/openai_example_streaming.py

Same task and same handlers as `openai_example_non_streaming.py`, driven
through `completion_stream`. What the streaming variant shows: OpenAI native
calls stream START → END only. Their in-progress payload arrives on the wire
as raw text (a diff being typed out, a command being typed out), not as JSON,
so there are no public `tool_call_delta` events — but
`tool_call_end.tool_call` is always the complete typed call.
...
for turn in range(1, MAX_TURNS + 1):
    ...
    with completion_stream(MODEL, messages, system_message=SYSTEM, tools=tools) as stream:
        for event in stream:
            if event.type == "text_delta":
                ...
                # Native calls emit no tool_call_delta in between — the next
                # event for this index is its tool_call_end.
                print(f"[call] {event.name}")
            elif event.type == "tool_call_end":
                print(f"       {type(event.tool_call).__name__} {event.tool_call.arguments}")
