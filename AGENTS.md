...
The demo needs a key for whichever provider it talks to — except with `--faux`. Either name it in `~/.local/share/luca/auth.json` (`{"openrouter": {"type": "api", "key": "sk-or-…"}}`), which the TUI reads at boot and hands to the runner, or leave that file out and let `luca.client` read the provider's own env var (`OPENROUTER_API_KEY`, …) from the environment. The TUI loads a `.env` at boot to fill that environment — nearest one at or above the cwd bounded by the repo, `LUCA_ENV_PATH` to name another, exported variables winning over the file, and an unreadable line stopping the launch with its line number (`luca/agent/contrib/tui/env_file.py`). The library itself never reads a file for credentials. The TUI also CONSTRUCTS the provider at boot (`check_provider_buildable`), so a missing region or a half-written credential fails at startup instead of on the first message; `/model` is still never a gate, it switches and reports. `auth.json` entries are discriminated on `type`: `"api"` is one opaque string, `"aws"` is the SigV4 tuple for Bedrock (`{"bedrock": {"type": "aws", "profile": "work"}}`, every field optional and filled from `~/.aws`). Bedrock takes either that or a Bedrock API key in `AWS_BEARER_TOKEN_BEDROCK`, which wins — see [docs/client/09-providers-and-transports.md](docs/client/09-providers-and-transports.md#bedrock-authentication). Never in `luca.json`, and never on the session: `LLMConfig` is persisted and copied onto every assistant message. Sessions persist to `~/.luca/projects/<encoded-project-path>/<session-id>.json`, one directory per project; `--resume` (or `/resume`) picks one back up, and `sessions.directory` in the config moves the root.
