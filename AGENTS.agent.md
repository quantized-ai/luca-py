...
- `status: ExecutionStatus` — the framework's execution lifecycle and ONLY that: `PENDING`, `RUNNING`, `COMPLETED`, `FAILED`, `NOT_FOUND`, `INVALID`, `REJECTED`, `REFUSED`, `CANCELLED`, `INTERRUPTED`, `TIMED_OUT`. `COMPLETED` means "the framework received a result", not "the tool succeeded".
