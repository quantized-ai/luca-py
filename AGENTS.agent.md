...
4. Step-limit / doom-loop checks → `prepare_llm_call()` (`before_llm_call`) → `_collect_tools()` (raced; a lost race skips the LLM call and returns to the loop top) → model call (reached only when every execution is terminal).
