...
│   ├── questions/       # the FIRST DEFERRED tool — the model asks, the turn parks
│   │   ├── __init__.py  # package surface: QuestionsTool, QuestionsPlugin,
│   │   │                #   Question, OptionsType
│   │   ├── tool.py      # QuestionsTool (`ask_user`): execute() as a pure
│   │   │                #   predicate + the app-facing pending/is_answered/
│   │   │                #   answer + the overridable renderer; store keyed by
│   │   │                #   tool_call_id, JSON-shaped, accumulating
│   │   └── plugin.py    # QuestionsPlugin — owns the store and ONE tool instance
...
└── core/
    ...
    │                    #   ExecutionDeferred + ExecutionAttempt(Outcome),
...
- `status: ExecutionStatus` — the framework's execution lifecycle and ONLY that: `RECEIVED`, `PENDING`, `RUNNING`, `AWAITING_RESULT`, `COMPLETED`, `FAILED`, `NOT_FOUND`, `INVALID`, `REJECTED`, `REFUSED`, `CANCELLED`, `INTERRUPTED`, `TIMED_OUT`. `RECEIVED` is the pre-birth state: the model asked for the call and the runner appended the entry, but the registry has not answered yet. `AWAITING_RESULT` (0007) is the PARKED state: the body was dispatched and returned `ExecutionDeferred` — "I cannot produce the final result yet" — so only the application can move it. `COMPLETED` means "the framework received a result", not "the tool succeeded". `NONTERMINAL_STATUSES` (RECEIVED/PENDING/RUNNING/AWAITING_RESULT) is the single source of truth for every "still advanceable" rule — projection, pruning, context accounting.
...
- Lifecycle timestamps: `created_at` (inherited from `Entry`, stamped at commit) is the BIRTH stamp, `finished_at` every terminal transition, `cancel_signalled_at` run cancellation only (a deadline never sets it). `updated_at` is ledger bookkeeping, not timing. There is no `started_at` on the execution any more — see the timestamp rework below.
- `attempts: list[ExecutionAttempt]` (`outcome`, `started_at`, `ended_at`; outcomes DEFERRED / COMPLETED / FAILED / INTERRUPTED / TIMED_OUT / DISCARDED) — AUDIT ONLY. It derives nothing and is UNBOUNDED (no cap, no fold; an application that wants to trim does it in `after_tool_execution`, which runs once, when the list is final). One entry per BODY INVOCATION, appended when the execution is persisted RUNNING — after `prepare()` returned, before the callable is invoked. That placement IS the invariant "a `prepare()` that raises appends no attempt", true on the first dispatch and the fifth alike. `outcome is None` means the attempt is still in flight, or the process died mid-flight — orphan recovery closes that one DISCARDED, which is a different fact from INTERRUPTED: INTERRUPTED means the framework killed the body and knows it did not finish, DISCARDED means the process died and nobody ever saw how that run went.

**A tool may answer "not yet".** A prepared callable can return `ExecutionDeferred()` in place of an `ExecutionResult` — an EMPTY Pydantic marker, deliberately: the core learns exactly one new fact and nothing more, because a tool's durable state has a home already (`AgentSession.extras`, handed to the tool at construction, keyed by `tool_call_id`). The execution parks at `AWAITING_RESULT` and the drive returns. THERE IS NO RESUME, ONLY RE-DISPATCH: when a tool defers, nothing about that call stays alive — no callable, no coroutine, no runner state — so the next drive runs the identical dispatch path from scratch, `before_tool_execution` → `ToolRegistry.prepare()` → a brand-new callable → invoke. `prepare()` therefore re-runs every poll (it was already contractually re-callable and idempotent; that is now load-bearing rather than merely correct), `status` cycles `AWAITING_RESULT → RUNNING → AWAITING_RESULT`, and `ToolExecutionStarted` fires once per poll. Approval is NOT re-asked per poll — the decide step only ever sees PENDING executions, so a call approved once stays approved. It is ONE tool call, with N attempts. NOTHING BOUNDS A DEFERRAL: re-dispatching produces no model round, so no step limit and no doom-loop check trips; the driver owns the cadence, and its handler MUST BLOCK UNTIL IT HAS MADE PROGRESS, because a handler that returns without resolving anything causes an immediate re-drive, another deferral, and a spin as fast as Python runs. Nothing cleans `AgentSession.extras` after a cancelled or crashed call either — the driver owns resolution, so it owns abandonment. Explicit, not an oversight.

**The timestamps were reworked for it, on `ToolExecution` only** (`CompactionEntry` is untouched). One call can now be dispatched many times, so there is no single dispatch to stamp: `started_at` LEFT the execution for the attempt, `ended_at` was renamed `finished_at`, and the two derived properties changed MEANING — `dispatched` is now `bool(attempts)`, and `duration_ms` is `finished_at - created_at`, TOTAL time outstanding with the approval wait and any parked time included, NOT body wall-clock (that is per-attempt: `attempts[-1]`, or the sum). That change of meaning is deliberate and affects every existing consumer.
...
Projecting a conversation that contains a nonterminal `ToolExecution` raises `ProjectionError` — the runtime must never call the model while an execution is nonterminal — with EXACTLY TWO carve-outs, and they are the same kind of thing: a durable resting state only the application can move, not a runtime in flight. A GATED execution (0008 — `PENDING` with `approval_status=PENDING`) projects a placeholder `ToolMessage` carrying `ConversationProjector.AWAITING_APPROVAL_OUTPUT`, and a PARKED one (0007 — `AWAITING_RESULT`) one carrying `AWAITING_RESULT_OUTPUT`. Both are `is_error=False` — the call has not failed, and an error result is exactly what makes a model retry — and both are replaced by the real result at the same path position once the application answers, making them the only projected tool messages that are not final. The placeholder is what lets a message posted while blocked reach the model at all, and ONLY a user POST ever forces either projection: compaction skips any conversation with an open turn, pruning refuses, the context manager leaves the entry at zero tokens. `AWAITING_RESULT_OUTPUT`'s wording is HEAVIER than the gate's on purpose — a gate can be terse because the model has no available action, but here it does, and a re-call mints a NEW `ToolExecution` under a new `tool_call_id`, leaving the tool holding two open jobs for one decision. `PENDING` with approval `None`/`ALLOWED`, and `RECEIVED` / `RUNNING`, still raise. Missing entry ids, unknown entry types, and a COMPLETED execution without a result fail the same way; projection never invents fallback content. An UNRESOLVED `ChildConversation` is legal inside the OPEN turn (it renders nothing — the model tracks its tasks through the spawn confirmations and the updates) and fails loud anywhere else: no close may leave an unresolved child behind, so that state is a framework bug or hand-authored corruption.
...
1. `await run` — drives/joins to the next stopping point and returns a `RunResult` (status + outcome + `pending_approvals` + `pending_deferred_tool_executions`; no usage — read `AgentSession.usages`).
...
| `BLOCKED` | stop again immediately; you must act first | yes — the conversation derives `BUSY` and the next drive answers it past the gate or the parked call, then re-parks |
...
- open turn with UNRESOLVED CHILDREN → its own sub-order, because a parent can advance for more reasons than its own executions: (1) an advanceable execution (`RECEIVED` / `RUNNING` / `PENDING` with approval `None`/`ALLOWED`) → `BUSY`; (2) any child that can advance (BUSY / IDLE / CANCELLING) → `BUSY`; (3) a gated OR PARKED execution with no unseen user post → `BLOCKED` — ranked ABOVE the material term, because the next drive can only re-park and `BUSY` would hot-loop a poll-for-BLOCKED consumer. A parked call has to be NAMED here rather than left to fall into the material term: any completed sibling satisfies that term, so a drive that can only re-park would derive `BUSY`. An unseen POST (`open_turn_unseen_post` — a `UserMessage` after the open turn's last `AssistantMessage`, deliberately narrower than the material predicate) lets both yield (0008), since the placeholder makes the next drive able to answer it; (4) unseen material (`open_turn_unseen_material` — a posted message or a terminal non-spawn-declaring execution after the last assistant message, the resolved child's result execution included; EXCLUDED under `wake_parent_on_subagent_completion=False`, which the derivation passes as the predicate's `include_child_results=` so status and drive stay in agreement) → `BUSY`: the next run() calls the model with it; (5) else `BLOCKED`.
- open turn with something runnable → `BUSY`. Runnable means: a `RECEIVED` execution (the next drive births it), or an orphaned `RUNNING` execution (the next drive recovers it to `INTERRUPTED`, no re-dispatch), or a `PENDING` execution whose `approval_status` is `None` (crash mid-decide — `run()` self-heals by asking again) or `ALLOWED` (dispatchable), or no nonterminal execution at all (the model can be called). An `AWAITING_RESULT` execution is listed EXPLICITLY as not-runnable rather than left to fall through, and that is the trap the predicate hides: the fallback reads "nothing is left except gates", and a parked call is not a gate — omitting it derives `BUSY` for a drive that can only park and return, which spins a `while not idle(): run()` driver.
- open turn with nothing runnable but an unseen user post (`open_turn_unseen_post`) → `BUSY` (0008): only gated or parked executions remain, but the placeholder lets the next drive answer the post and re-park.
- open turn with nothing runnable → `BLOCKED`. That is: only gated or parked executions remain — a parked tool call is the SECOND reason a conversation is `BLOCKED`, and it is answered through `pending_deferred_tool_executions()` exactly as a gate is answered through `pending_approvals()`.
...
- PARKED executions (`AWAITING_RESULT`) → `INTERRUPTED`, same stamps. INTERRUPTED and not CANCELLED because the body demonstrably STARTED, and CANCELLED is defined as "cancellation prevented the body from starting". The last attempt is already closed DEFERRED, so a call cancelled while parked records exactly ONE attempt — nothing was running when the cancel landed. The tool is never told.
...
async def run(*, cancellation_token: CancellationToken) -> ExecutionResult | ExecutionDeferred: ...
```
...
- **`prepare`** resolves the tool and validates the arguments, then returns a CALLABLE that runs the body — it must not run it. Raising means the body never runs and the execution is never marked RUNNING: `ToolNotFound` → NOT_FOUND, `InvalidToolArguments`/pydantic `ValidationError` → INVALID, anything else → FAILED, all with NO `ExecutionAttempt` appended (`dispatched` stays False). Once the callable is invoked, every raise is FAILED. A returned `ExecutionResult` → COMPLETED (after `ContextManager.process_tool_output`); a returned `ExecutionDeferred` → AWAITING_RESULT, no outcome at all. It is called once per DISPATCH ATTEMPT and only for an already-approved execution — which is once per call for an ordinary tool and once per poll for a deferring one, so the re-callable/idempotent rule is now load-bearing rather than merely correct.

The arguments are easy to misuse in opposite directions. The **`AgentSession` is the LIVE object** — the same instance the runner and ledger write through — so a registry may hold it and re-read current state later, including from inside a prepared callable. The **`ToolExecution` is a SNAPSHOT**, detached as of the moment the call was made; the runner persists RUNNING and opens the `ExecutionAttempt` AFTER `prepare()` returns, so a reference captured during `prepare()` is already stale. Capture what the callable needs during `prepare()`. And the session is **read-only to every implementation** — the runner owns every write, `session.tool_specs` included.
...
**Dispatch is PREPARE, then run** (per ready execution — `approval_status` ALLOWED and status `PENDING` (never dispatched) or `AWAITING_RESULT` (dispatched, deferred, re-dispatched FROM SCRATCH), in order):
...
3. A raise, or a return that is not callable, terminalizes the execution WITHOUT it ever being marked RUNNING — no attempt appended, no `ToolExecutionStarted`, `details["phase"] == "prepare"`. That phrasing holds on a RE-dispatch too, where the execution demonstrably was dispatched before: the invariant is about the ATTEMPT LIST, not about a single `started_at`. For the not-callable case the runner synthesizes the failure itself.
...
5. Otherwise persist `RUNNING` with a fresh open `ExecutionAttempt` (the birth `tool_spec` stands — no dispatch-time re-snapshot), emit `ToolExecutionStarted`, and invoke the callable under the cancellation race + deadline (`tool_spec.timeout_in_ms`, else `RuntimeConfig.tool_execution_timeout_in_ms`). The INVOCATION sits inside the failure handling: a callable returning a plain value has already been invoked, so it records as a post-dispatch failure rather than crashing the run.
6. A returned `ExecutionDeferred` is NOT an outcome and SKIPS THE SHARED TAIL entirely: the attempt closes DEFERRED, `AWAITING_RESULT` is persisted, and nothing else fires — no `ToolExecuted`, no `after_tool_execution`, no `process_tool_output`, no `finished_at`. The call has not finished; the drive's park branch takes it from here.

A returned result is `COMPLETED` (whatever `result.is_error` says); every post-dispatch raise is `FAILED`, with a structured `ToolExecutionError` built by the overridable `runner.to_tool_execution_error(execution, exception, *, phase)`; deadline expiry is `TIMED_OUT` (the deadline bounds ONE poll, never the parked period); grace expiry is `INTERRUPTED`. `after_tool_execution(execution, exception)` observes EVERY outcome before the final persist (registry-authored terminal births carry no live exception).

A RUNTIME-MINTED call may not defer — `_invoke_runtime_tool` dispatches with `allow_deferral=False`, and step 6 becomes an ordinary FAILED tool call with the attempt still recorded DEFERRED (what the body actually did; otherwise a refused deferral would be indistinguishable from a body that raised). The reason is structural: `_derive_child_result` mints a fresh `ToolCall` per pass and only resolves the `ChildConversation` on `ToolExecuted`, so a deferral there would never resolve the link, the next iteration would mint ANOTHER result execution, and the progress-continue would spin — growing the entry store with no LLM cost and no limit to trip.

`AgentSessionRunner.pending_deferred_tool_executions(conversation_id=None)` is how the application finds parked calls — SUBTREE-SCOPED exactly like `pending_approvals()`, attribution through each execution's own `conversation_id`, and a plain read over the session, so it is safe to call from another task while a run is live (which is how a parked SUBAGENT is discovered — a live parent run leaves no between-drives gap to poll in). Deliberately a POLL rather than an event: `ToolExecutionStarted` fires per dispatch attempt and `ToolExecuted` only at the end, so a consumer reading the stream alone cannot tell parked from in flight. NO EVENT ANNOUNCES A PARKED CALL; `run.notify(execution)` covers deferrals unchanged.
...
`AgentSessionRunner.rewind_to(entry_id)` is the SECOND caller of `transition_conversation`, and the only other thing in core that installs a successor conversation. It archives the main conversation and installs one over the path truncated so `entry_id` is the last node (`None` = an empty path); rewinding to the current leaf is a no-op. It creates no entries, so no `before_entry_written` middleware runs and there is nothing to thread — strictly simpler than the compaction commit.

Four guards, all checked before anything is written: a LIVE RUN on the main conversation (a transition re-points the row a drive holds in a local — `_rebind_run` exists for exactly that moment); an OPEN TURN (the `schedule_compaction` rule, for a related reason — discarding in-flight or parked work silently would strand a gate the registry still answers for); an `entry_id` off the path; and a cut INSIDE a turn bracket, tested by deriving `open_turn_index` over the resulting prefix rather than by a `TurnStart` check, so a trailing queued `UserMessage` is droppable while a mid-bracket cut is refused (it would carry an `AssistantMessage` whose `ToolExecution` nodes were dropped — a `tool_use` with no correlated result).

POLICY-FREE, and that is the whole point: core answers "make the conversation be this prefix again" and nothing else. What a checkpoint IS, when one is taken and what else is restored alongside it belong to the application — `luca/agent/contrib/checkpoints/` is the shipped one (a shadow git repo for the workspace, an index on `AgentSession.extras`, `/undo` and `/rewind` in the TUI). A `ChildConversation` in the dropped span leaves its subagent conversation in the catalog as inert history; usage records on the archived conversation are untouched, so undoing a turn does not refund it. Details in `docs/agent/15-rewind.md`.
...
- Override `async _execute(args, session, conversation_id, *, tool_name, tool_call_id, cancellation_token) -> str` for simple tools, or `async execute(...) -> ExecutionResult | ExecutionDeferred` for rich output (`is_error`, `metadata`, multi-block) or a deferral. An `is_error=True` result still records `COMPLETED` — `is_error` is the tool's own verdict, not a lifecycle fact.
- Both receive the LIVE `AgentSession` (read `session.id` / `session.session_config.llm_config`; treat it as READ-ONLY — the runner owns every write), the `conversation_id` the call belongs to (key any per-conversation state by it — a tool instance is shared by every conversation), the keyword-only `tool_name` / `tool_call_id` (WHICH CALL this is — the effective name the registry resolved under, which need not be the class's `name`, and the key for per-call state and for `session.get_tool_execution(tool_call_id)`), and the keyword-only `CancellationToken`. Per-run application state is not the framework's concern: a tool is application code and can close over its own references or read a `contextvars.ContextVar`.
- `execute` may return `ExecutionDeferred()` — "not yet". The execution parks at `AWAITING_RESULT`, the drive returns, and the application resolves whatever the tool is waiting on (it finds the parked calls through `AgentSessionRunner.pending_deferred_tool_executions()`) before driving again. There is no resume: the next drive RE-DISPATCHES from scratch, so `execute` must be a PURE PREDICATE — read state, answer ready / not yet, never wait. `tool_call_id` is the only thing connecting two invocations of one call, and `AgentSession.extras` is where the state goes when it must survive a restart. The return union is WIDE ON THE BASE deliberately: a subclass returning a wider type than its base is an LSP violation type-checkers flag. `luca/agent/contrib/questions/` is the reference deferring tool.
- `SimpleToolRegistry` validates LLM-produced arguments through `Args` at birth (for the birth status) and again in `prepare()` (for dispatch). Malformed args become a terminal `INVALID` execution with a structured `ToolExecutionError` and never reach `decide()`. Its `prepare()` binds `tool_name` / `tool_call_id` into the closure alongside the validated args, the session and the conversation — `PreparedTool` still takes only the token, so the core still depends on no Python tool class.
- Timing is recorded by the runner on the execution, never on the result: `created_at`/`finished_at` bracket the whole call, and one `ExecutionAttempt` per body invocation carries that invocation's own pair. `timeout_in_ms` bounds ONE body invocation only — never the parked period, which nothing ages out.
...
- `tool()` / `tool_class()` build one from plain callables for runtime-constructed tools; subclassing stays the recommended mechanism. The factories ACCEPT AND DROP `tool_name` / `tool_call_id`: the wrapped callable keeps the three-argument `(args, session, conversation_id) -> str` shape they advertise, and they only ever wire the text path — so a factory-built tool can neither see which call it is nor defer. Hand-write the class for either.
...
-1. (drive start) `_recover_orphans`: any persisted `RUNNING` execution → `INTERRUPTED`, its dangling `ExecutionAttempt` closed `DISCARDED` (`after_tool_execution` runs, no re-dispatch), before the flush too. INTERRUPTED means the framework killed the body and knows it did not finish; DISCARDED means the process died and nobody ever saw how that run went.
...
2. Ready executions (`ALLOWED` + either `PENDING` or `AWAITING_RESULT` — `open_turn_ready_executions` is what re-selects a parked call for its next poll) → `_dispatch_batch` (sequential by choice — the state model is parallel-ready; a token already tripped when an execution's turn comes up stops the batch, leaving it and its successors untouched for the wind-down).
...
3b. A POST REACHES THE MODEL PAST THE GATE (0008): with an unseen user post (`_has_unseen_post` — the durable `open_turn_unseen_post` plus the drive-local `last_seen` fingerprint), the drive falls through to step 4 instead of parking, deliberately PAST the progress-`continue` (`undecided` holds the gate on every pass, so continuing would re-ask `decide()` in a tight loop). ONLY a post does this — an allowed sibling's result completed in the same round is not a wake. The close site refuses to close COMPLETED while the gate is live (`has_awaiting_approval` is its third term), and both failure closes (`hard_max_steps`, an LLM failure) settle gated executions first via `_settle_open_executions` — no close leaves a nonterminal execution behind.
3b'. A PARKED TOOL PARKS THE DRIVE (0007) — a copy of the gate branch minus the announcement, since no event says "parked" and the driver polls `pending_deferred_tool_executions()` after the drive returns. PLACEMENT IS LOAD-BEARING TWICE. Ahead of the progress-`continue`, because a re-dispatched deferral IS progress by that test: a branch after it would re-select the same execution, invoke the body again, and spin as fast as Python runs — no LLM cost, no limit tripped. And ahead of 3c, whose wake rule counts any terminal execution as fresh material, so a drive reaching it with a call still parked would call the model because a SIBLING completed, with no post anywhere; sitting here is what keeps "a post is the only thing that projects a parked call" true. The gate arm is checked FIRST on purpose — a gate and a deferral can be open in the same drive, and the gate's announcement must still happen even when the deferral is what would park. An unseen post falls through to step 4 exactly as at a gate, and the COMPLETED close site refuses to close over a parked call like it does over a live gate.
3c. Otherwise, if the open turn still has an unresolved child AND no unseen material (`open_turn_unseen_material`, plus the drive-local `seen` fingerprint for a post that landed under this drive's own in-flight LLM call) → `_await_subtree`; with material the drive falls through to step 4 and calls the model — the wake round. EVERY park site (3, 3b', 3c) releases the conversation's worker-pool slot on entry and re-acquires it on wake (raced against the token) — parked is not working, which is what lets a deferral at `subagents_max_workers=1` admit a queued sibling. All obey ONE rule: **a drive returns only when nothing in its SUBTREE can advance**. A gated child with no subtree returns (its drive is gone; `notify()` or the next `run()` restarts it); a gated parent whose children still work waits — which is why `ApprovalRequired` is not terminal on a parent's stream.
...
| `_dispatch_one` / `_prepare_tool` / `_run_tool_body` | `before_tool_execution` (its `raw_tool_call` is the effective call) → `tool_registry.prepare(session, execution)` raced (grace 0) → on raise/non-callable: terminal with `phase="prepare"`, NO `RUNNING`, NO attempt, NO `ToolExecutionStarted` → on cancel: `CANCELLED` in place → else persist `RUNNING` + a fresh open `ExecutionAttempt` (birth `tool_spec` stands) → `ToolExecutionStarted` → invoke the callable (the `ensure_future` INSIDE the failure handling) under token race + outside deadline (`tool_spec.timeout_in_ms`, else config) → `COMPLETED`/`FAILED`/`INTERRUPTED`/`TIMED_OUT`, or `AWAITING_RESULT` on an `ExecutionDeferred`, which closes the attempt DEFERRED, persists the park and RETURNS WITHOUT the shared tail; a mid-body cancel persists `cancel_signalled_at` before the grace wait. `allow_deferral=False` (runtime-minted calls only) turns the deferral into an ordinary FAILED, attempt still DEFERRED |
| `_finalize_outcome` | The ONE outcome tail, for every terminal execution whether or not it dispatched: close the open attempt (`attempt_outcome=`, defaulted from the status; a no-op when the last attempt is already closed — which is why a cancel over a parked call records exactly one) → recalculate context → `after_tool_execution(execution, exception)` → persist → `ToolExecuted` built from the projector. A DEFERRAL never reaches here |
| `_settle_open_executions` | The close-side settle, SHARED by the cancel wind-down, `hard_max_steps` and a failed LLM call (renamed from `_settle_undispatched` when it grew its third bucket): undispatched (`RECEIVED`/`PENDING`) → `CANCELLED`, parked (`AWAITING_RESULT`) → `INTERRUPTED`, both stamped `cancel_signalled_at`. A parked bucket in the cancel path alone would strand the execution outside any open turn on the other two and freeze its placeholder into the projected history forever |
| `to_tool_execution_error` | PUBLIC override point: live exception → durable `ToolExecutionError` (type + message; pydantic errors under `details.errors`; `details.phase` is a FACT passed by the call site — `create_execution` / `prepare` / `execution` — not inferred from the attempt list) |
...
| `tests/agent/test_runner_failures.py` | The prepare/dispatch outcome table (every `prepare()` failure mode, the non-callable and non-awaitable guards, `attempts`/`dispatched`/`details["phase"]`), tool deadlines and their scope, crash recovery (orphaned RUNNING), LLM failure closes |
...
| `tests/agent/test_runner_rewind.py` | `rewind_to`: the transition (successor installed, predecessor archived with its whole path, no entry mutated, usages preserved), the no-op at the leaf, the empty rewind, a dropped trailing `UserMessage`, an inert subagent link, and the four guards refusing before anything is written |
| `tests/agent/test_deferred_tools.py` | Deferred tool calling on ONE conversation: the park itself (`AWAITING_RESULT`, one closed `ExecutionAttempt`, no `finished_at`, no `ToolExecuted`, 0 context tokens), the two load-bearing placements of the park branch, the three status terms, approval asked once per CALL, the projection carve-out and a post past a parked call, and the shared close-side settle (cancel / `hard_max_steps` / LLM failure) |
| `tests/agent/test_deferred_tools_timing.py` | The park under the two clocks: a poll that hangs past the deadline is TERMINAL and the attempt list says which poll died, a deferral returned inside the cancel grace window still parks, and the parked period is unbounded. Never races two timed things |
| `tests/agent/subagents/test_deferred_tools.py` | What a TREE makes a different question: subtree-scoped discovery while the parent run is live, the third status term in the subagent branch, re-dispatch from outside (`notify()` / `_restart_unresolved_children`), and a parked subagent holding no worker slot |
| `tests/agent/test_deferred_native_tools.py` | A PARKED provider-native call on the wire: the `shell_call_output` synthesizer has no "pending" form, so a parked call reaches the model as exit 1 with the placeholder in `stderr` — a recorded compromise, unreachable with the four shipped (local, synchronous) native tools |
...
| `tests/agent/contrib/mcp/` | Self-scoped contrib tests for the MCP client: the pure layers (the `luca.json` block incl. the cross-file merge that produces a two-transport server, the three server hashes, the base64 sentinel and `x-mcp-header` rules, the SSE decoder across every chunking of one stream, JSON-RPC framing per era, the wire→core mapping), the connection and service against a real stdio fixture (`server_fixture.py`, scriptable via argv), Streamable HTTP over `httpx.MockTransport`, and OAuth — no runner |
| `tests/agent/contrib/tui/test_mcp.py` | The TUI half: the service is app-scoped, so `/clear` keeps its connections, catalog and tokens, and `_close_plugins` does not close them |
## When in doubt
...
| Deferred tool calling (the park, re-dispatch, the driver contract) | `luca/agent/core/runner.py` (`_dispatch_one` step 6 + the drive's 3b' park branch) + design principles 4, 6 and 11 above + `luca/agent/contrib/questions/` |
