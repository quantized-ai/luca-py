...
async def test_missing_envelope_markers_fail_verification(tmp_path, run):
    ...
    assert result.is_error is True
    assert body(result) == ("Invalid patch: missing '*** Begin Patch' / '*** End Patch' markers")
...
async def test_add_update_and_delete_in_one_patch(tmp_path, run):
    ...
    assert body(result) == ("Success. Updated the following files:\nA nested/new.txt\nM modify.txt\nD delete.txt")
...
async def test_update_or_delete_against_a_directory_fails(tmp_path, run):
    ...
    assert update.content[0].text == (f"Update target is a directory, not a file: {tmp_path / 'adir'}")
...
def test_permission_resource_lists_all_touched_paths(tmp_path, perm):
    ...
    assert access.metadata["preview"] == (f"Access directories {tmp_path}, {tmp_path / 'src'}")
    ...
    assert request.resources == [
        ResourcePermission(
            ...
            resource=str(tmp_path / "hello.txt"),
        ),
        ResourcePermission(
            permission="apply_patch",
            resource=str(tmp_path / "src/app.py"),
        ),
        ResourcePermission(
            permission="apply_patch",
            resource=str(tmp_path / "src/main.py"),
        ),
        ResourcePermission(
            permission="apply_patch",
            resource=str(tmp_path / "obsolete.txt"),
        ),
    ]
    assert request.metadata["preview"] == ("Apply patch to hello.txt, src/app.py, src/main.py, obsolete.txt")
