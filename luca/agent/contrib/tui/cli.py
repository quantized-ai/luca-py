...
def _mcp_servers(mcp_servers: dict):
    """The configured MCP servers, or None. Verifies the optional `mcp` extra is
    installed when servers are configured (the SDK is imported lazily, so an
    install without the extra is fine until a server is actually configured)."""
    if not mcp_servers:
        return None
    try:
        from luca.agent.contrib.mcp import build_mcp_plugin  # noqa: F401  (forces the SDK import)
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        mcp_servers = _mcp_servers(config.mcp)
